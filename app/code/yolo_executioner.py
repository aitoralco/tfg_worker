...
# seguro al limitar numero de cores
...
logging.info(f"CONFIRMED: PyTorch is using {torch.get_num_threads()} thread(s)")
...
class YoloExecutioner:
    def __init__(self, models_path):
        ...
        self.logger.info(
            "YoloExecutioner initialized with models path: %s", models_path
        )
    ...
    def __load_model(self, model_source):
        ...
        if os.path.isabs(model_source) or os.path.exists(model_source):
            self.logger.info(
                "Model source is an absolute path or exists: %s", model_source
            )
            return YOLO(model_source)

        # check if model exists in local models directory
        local_path = os.path.join(self.models_path, model_source)
        if os.path.exists(local_path):
            self.logger.info("Model in local directory: %s", local_path)
            return YOLO(local_path)

        # if not local, treat as model name for auto-download
        self.logger.info("Model not locally. Attempting download")
        return YOLO(model_source)
    ...
    def train(
        ...
        model_object.train(
            data=data if data != '' else yaml_path,
            epochs=epochs,
            batch=batch,
            imgsz=imgsz,
            workers=workers,
